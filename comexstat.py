...
class ComexStat:
    ...
    def __init__(self):
            ...
            self.CERT_PATH: str = os.path.join(os.path.dirname(__file__), 'certs', 'mdic-gov-br.pem')

    def _make_request(self, method: str, endpoint: str, params: Optional[Dict[str, Any]] = None, json_body: Optional[Dict[str, Any]] = None) -> Optional[requests.Response]:
        """
        ...
        try:
            if method.upper() == "GET":
                ...
                response = requests.post(url, json=json_body, params=params, verify=self.CERT_PATH)
            else:
    ...
    def get_last_updated_date(self, data_type: str = "general") -> Optional[Dict[str, Any]]:
    ...
    def get_available_filters(self, data_type: str = "general", language: str = "pt") -> Optional[pl.DataFrame]:
        endpoint = f"/{data_type}/filters"
        params = {"language": language}
        response = self._make_request("GET", endpoint, params=params)
        return pl.DataFrame(response.json()["data"]['list']) if response else None

    def get_filter_values(self, filter_name: str, data_type: str = "general", language: str = "pt") -> Optional[pl.DataFrame]:
        endpoint = f"/{data_type}/filters/{filter_name}"
        params = {"language": language}
        response = self._make_request("GET", endpoint, params=params)
        return pl.DataFrame(response.json()["data"][0]) if response else None

    def get_available_details(self, data_type: str = "general", language: str = "pt") -> Optional[pl.DataFrame]:
        endpoint = f"/{data_type}/details"
        params = {"language": language}
        response = self._make_request("GET", endpoint, params=params)
        return pl.DataFrame(response.json()["data"]['list']) if response else None

    def get_available_metrics(self, data_type: str = "general", language: str = "pt") -> Optional[pl.DataFrame]:
        endpoint = f"/{data_type}/metrics"
        params = {"language": language}
        response = self._make_request("GET", endpoint, params=params)
        return pl.DataFrame(response.json()["data"]['list']) if response else None

    def query_comexstat_data(self, flow: str, period_from: str, period_to: str, data_type: str = "general", filters: Optional[List[Dict[str, Any]]] = None, details: Optional[List[str]] = None, metrics: Optional[List[str]] = None, language: str = "pt", month_detail: bool = False) -> Optional[pl.DataFrame]:
        """
    ...
    def get_auxiliary_table(self, table_name: str, language: str = "pt", page: int = 1, per_page: int = 100, add: Optional[str] = None, search: Optional[str] = None) -> Optional[pl.DataFrame]:
    ...
    def fetch_comexstat_by_city(self, year: int, state_code: int) -> Optional[pl.DataFrame]:
        """
