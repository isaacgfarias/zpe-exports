...
class Comtrade:
    ...
    def _get_key(self, key=None) -> str:
        if key is None:
            ...
            if not key:
                raise ValueError('No subscription key provided and \'COMTRADE_API_KEY\' not found in environment.')
        return key


    def query_data(self, save_csv=False, max_records=None, **filters:Dict[str, Any]) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df = comtrade.getFinalData(subscription_key=self.comtrade_key, **final_filters)
